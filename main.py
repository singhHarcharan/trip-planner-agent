...
data = service.get_response_from_llm(prompt)
print("Response from LLM:")
print(data)
print("\n")

print("1.) Fetching response from LLM for the prompt:")
print("Response from LLM:")
print(data)
print("\n")

# 2.) Function call to get relevant date based on the weather by calling weather API
dates = service.get_relevant_dates_based_on_weather(data['destination'], data['condition'])
...
available_dates = service.get_available_dates(data['source'], data['destination'], dates)
print("Available dates based on your calendar:")
print(available_dates)
print("\n")

# 4.) Function to Search for hotels with rating > 4 on available sunny days.
hotels = service.search_hotels(data['destination'], available_dates)
...
flight_booking = service.book_flight(data['source'], data['destination'], available_dates)
print("Flight booking details:")
print(flight_booking)
print("\n")

# 6.) Function to book a hotel on available dates.
hotel_booking = service.book_hotel(data['destination'], available_dates)
print("Hotel booking details:")
print(hotel_booking)
print("\n")

# 7.) Function to send confirmation email with flight and hotel details.
confirmation_email = service.send_confirmation_email(data['source'], data['destination'], flight_booking, hotel_booking)
